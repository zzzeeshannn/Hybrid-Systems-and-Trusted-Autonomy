...
This program also runs the RK4 multiple times.
In each run, the step size is divided in half.
Process is repeated till error_estimate is < 1 ft.

Error estimate is defined as the difference in the final positions of the aircraft for time step h and h/2.
"""
...
def main():
    ...
    # Multiple RK4 simulation here
    # Initializing a high value of the difference
    counter = 1
    difference = 100
    while(difference > 1):
        print(f"------------------ RK STEP SIZE {step_size} -------------------------")
        # Initialize time counter
        start_time = time.perf_counter()
        # Calling RK method here
        states_rk,  final_position = run_rk4_classic_f16_sim(init, tmax, step_size, waypoints)
        if counter >= 2:
            difference = np.linalg.norm(previous_final_position - final_position)
        ...
        print(f"RK with step size: {step_size}, final state x, y, z: {round(x, 3)}, {round(y, 3)}, {round(z, 3)}, error estimate: {difference}")

        # Update step size and counter
        step_size /= 2
        counter += 1
...
def run_rk4_classic_f16_sim(init, tmax, step_size, waypoints):
    ...
    while current_time + 1e-6 < tmax:
        ...
        if (distance < waypoint_distance):
            ...
            print(f"For RK4 simulation of step size {step_size}, Waypoint changed at:- Time: {current_time} and distance: {new_distance}")
    ...
    # For calculating the error estimate, return the final position of the aircraft
    # Calculating the final position here
    ...
    return state_list, final_position
