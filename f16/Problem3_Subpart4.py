...
def main():
    ...
    # RK4 simulation here
    start_time = time.perf_counter()
    states_rk = run_rk4_f16_sim(init, tmax, step_size, waypoints)
    runtime = time.perf_counter() - start_time
    print(f"Simulation Completed in {round(runtime, 2)} seconds")
    ...
    filename = 'overhead_scipy_rk4.png'
    plt.savefig(filename)
    print(f"Made {filename}")
...
# Defining the RK method with scipy here
def run_rk4_f16_sim(init, t_max, step_size, waypoints):
    """
    This function simulates the system using the Runge Kutta method provided by the "Scipy" library.

    Main functions used here are:
    rk45.step(): Takes a step for the system
    rk45.dense_output(): Interpolation function
    ...
    rk45 = RK45(autopilot.der_func, 0, current_state, t_max, rtol = rtol, atol = atol)

    while current_time + 1e-3 < t_max:
        current_point = states[-1]

        rk45.step()

        interpolation_function = rk45.dense_output()
        previous_time = rk45.t_old
        current_time = rk45.t
        delta = current_time - previous_time

        for step in range(10):
            temp_time = previous_time + delta * step / 10
            next_point = interpolation_function(temp_time)
            ...
            if (distance < waypoint_distance):
                ...
                if (current_waypoint_index > 3):
                    return states
                autopilot = WaypointAutopilot(waypoints[current_waypoint_index])
                rk45 = RK45(autopilot.der_func, current_time, current_state, t_max, rtol=rtol, atol=atol)
                states.append(new_point)
            else:
                states.append(next_point)
